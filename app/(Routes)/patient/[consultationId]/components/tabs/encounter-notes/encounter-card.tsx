...
export function EncounterCard({
  ...
  // Show empty state card with "Add" button only if isConsultationView
  if (!hasEncounterData && !showForm) {
    return (
      <EmptyEncounterState 
        onAdd={() => setShowForm(true)} 
        showAddButton={isConsultationView}
      />
    );
  }

  // Show form for adding new encounter
  if (showForm) {
    return (
      <EncounterForm
  ...
  return (
    <Card className="border-b-2 border-b-[#E58116] mb-6">
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-3 text-sm text-muted-foreground">
            ...
            {/* Clinical Actions Dropdown - only show if consultation view and user is the doctor */}
            {isConsultationView && currentUserId === encounter?.consultation.doctor.id && (
              <DropdownMenu>
      ...
      <CardContent className="space-y-4 pb-6">
        {/* Add New Note button - only show if consultation view and user is the doctor */}
        {isConsultationView && currentUserId === encounter?.consultation.doctor.id && (
        ...
        {!isConsultationView && <h3 className="font-bold">Encounter Note</h3>}
      ...
      <CardContent className="flex flex-col items-center justify-center py-9">
        ...
        <p className="text-muted-foreground mb-4 text-center">
          {showAddButton 
            ...
            : "No encounter notes have been recorded for this patient."
          }
        </p>
        {showAddButton && <p className="font-medium mb-6">This is a normal text encounter!</p>}
        {showAddButton && (
          <Button
            onClick={onAdd}
